...
export async function POST(req: NextRequest) {
  ...
  if (mail) {
    try {
const dbTpl = renderDbTemplate("magic_link", { appName: mail.appName, link, email });
      await mail.transporter.sendMail({
        from: mail.from, to: email,
        subject: dbTpl?.subject || `Sign in to ${mail.appName}`,
        html: dbTpl?.html || emailTemplate({ appName: mail.appName, title: "Sign in to your account", body: "Click the button below to sign in. This link is valid for 15 minutes and can only be used once.", buttonText: "Sign In Now", buttonUrl: link, footer: `Sent to ${email} · If you didn't request this, ignore it.` }),
      });
